...
$(document).ready(function(){
	calculateDate($("#year").val());
    $("#year").change(function(){
    	calculateDate(this.value);
    });
});

/*
 * This function is calculate the 
 * fromDate and toDate from given 
 * selectedYear value.
 */
function calculateDate(selectedYear){
	if(selectedYear=='All'){
		selectedYear="2010-11";
	}
	var splitYears = selectedYear.split("-");
	var fromDate = "01/04/"+splitYears[0];
	var toDate = "31/03/"+""+(splitYears[0].substring(0,2))+""+splitYears[1];
	
	$('#fromDatepicker').data({date: fromDate});
	$('#fromDatepicker').datepicker('update');
	$('#fromDatepicker').datepicker().children('input').val(fromDate);
	
	$('#toDatepicker').data({date: toDate});
	$('#toDatepicker').datepicker('update');
	$('#toDatepicker').datepicker().children('input').val(toDate);
}
function submitClick() {
	...
	formAction = formAction +"&__title=Pending Report";
	...
	<div class="container">
		...
		<section class="panel" >
			<div class="panel-heading">
				<div class="row">
					<div class="col-md-12">
						<h4>Pending Summary Report Filter</h4>
					</div>
				</div>
			</div>
			<div class="panel-body">
					<birt:parameterPage id="birtParmPage" 
						reportDesign="report/PendingSummaryReport.rptdesign" name="pendingSummaryFilterForm"
						pattern="preview" format="pdf" title="Pending Summary Report" style="overflow: hidden;"
				 	...
				 	<div class="col-md-4">
				 		<spring:bind path="fromDate">
						  <div class="form-group ${status.error ? 'has-error' : ''}">
							...
							<div class="col-md-8">
								<div class="input-group date" id="fromDatepicker" data-provide="datepicker" data-date-format="${dispDateFormatter}" >
								 	<form:input path="fromDate" id="fromDate" class="form-control" placeholder="${dispDateFormatter}" readonly=""/>
				 	...
				 	<div class="col-md-4">
				 		<spring:bind path="toDate">
						  <div class="form-group  ${status.error ? 'has-error' : ''}">
							...
							<div class="col-md-8">
								<div class="input-group date" id="toDatepicker" data-provide="datepicker" data-date-format="${dispDateFormatter}" >
								 	<form:input path="toDate" id="toDate" class="form-control" placeholder="${dispDateFormatter}" readonly=""/>
				                 	<div class="input-group-addon">
				                 		<i class="fa fa-calendar" aria-hidden="true"></i>
				                 	</div>
				                 </div>
